...
# Set no other scopes and no other repositories. Deliberately NOT granted:
# Workflows/Actions write. The only step needing it is the dispatch fallback
# below, which runs exclusively when this token is ABSENT and uses GITHUB_TOKEN
# when it does — so granting it here would be a permanent right for a code path
# the token can never reach. If the secret is absent or expired the job falls
# back to GITHUB_TOKEN — degraded; see the steps below.
on:
  push:
    branches: [main]
permissions:
